...
const tabMeta: Record<WorkspaceTab, { label: string; title: string; description: string }> = {
  ...
  offers: { label: "Offerte", title: "Offerte", description: "Gestisci le tue quotazioni, la disponibilità e lo stato delle offerte della tua organizzazione." },
  ...
  collaborators: { label: "Collaboratori", title: "Collaboratori", description: "Gestisci le persone autorizzate a lavorare per la tua organizzazione." },
};
...
export default function Pr21PracticesDemo() {
  ...
  const toggleCollaborator = () => {
    setCollaboratorActive((current) => !current);
    setNotice(`Collaboratore demo ${collaboratorActive ? "disattivato" : "riattivato"}. Nessun dato reale modificato.`);
  };

  const renderOverview = () => (
    <>
      <section style={s.kpiGrid}>
        <article style={s.kpiCard}><span>Offerte online</span><strong style={s.kpiValue}>{offerStatus === "PUBBLICATA" ? 1 : 0}</strong><small>{offerStatus === "SOSPESA" ? "1 sospesa" : "0 in verifica"}</small></article>
        ...
        <article style={s.kpiCard}><span>Collaboratori</span><strong style={s.kpiValue}>{collaboratorActive ? 1 : 0}</strong><small>gestibili dal Partner Admin</small></article>
      </section>
  ...
  const renderOffers = () => (
    <section style={s.panel}>
      <div style={s.sectionHead}><div><span style={s.kicker}>LE TUE OFFERTE</span><h2 style={s.h2}>Quotazioni e pubblicazioni</h2></div><span style={s.count}>1</span></div>
  ...
  const renderCollaborators = () => (
    <section style={s.panel}>
      <div style={s.sectionHead}><div><span style={s.kicker}>COLLABORATORI</span><h2 style={s.h2}>Persone della tua società</h2></div><span style={s.count}>{collaboratorActive ? 1 : 0}</span></div>
      <article style={s.collaboratorCard}>
        ...
        <button type="button" style={s.secondary} onClick={toggleCollaborator}>{collaboratorActive ? "Disattiva" : "Riattiva"}</button>
      </article>
    </section>
  );
  ...
  return (
    <main style={s.page}>
      ...
      <div style={s.content}>
        ...
        <div style={s.safe}>DEMO SICURA · Le azioni modificano solo questa preview locale. Nessuna pratica, cliente, offerta, commissione, collaboratore o database reale viene modificato.</div>
...
const s: Record<string, React.CSSProperties> = {
  ...
  kpiGrid: { display: "grid", gridTemplateColumns: "repeat(4,minmax(0,1fr))", gap: 14 }, kpiCard: { display: "grid", gap: 8, padding: 20, borderRadius: 18, background: "#fff", border: "1px solid #dce6f1" }, kpiValue: { fontSize: 26 },
  ...
  sectionHead: { display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", marginBottom: 14 }, count: { minWidth: 32, height: 32, display: "grid", placeItems: "center", borderRadius: 999, background: "#eef6ff", color: "#0c5597", fontWeight: 900 }, h2: { margin: "7px 0 0", fontSize: 24 }, h3: { margin: "8px 0", fontSize: 22 },
  ...
  offerCard: { padding: 20, border: "1px solid #dbe6f0", borderRadius: 15, background: "#fbfdff" }, offerMeta: { display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 16, marginTop: 20, paddingTop: 18, borderTop: "1px solid #e4eaf0" },
  commissionCard: { display: "grid", gridTemplateColumns: "1.3fr 1fr 1fr", gap: 20, alignItems: "center", padding: 20, border: "1px solid #dbe6f0", borderRadius: 15, background: "#fbfdff" }, collaboratorCard: { display: "grid", gridTemplateColumns: "1.3fr 1fr 1fr auto", gap: 20, alignItems: "center", padding: 20, border: "1px solid #dbe6f0", borderRadius: 15, background: "#fbfdff" }, infoBox: { padding: 16, borderRadius: 12, background: "#eef6ff", color: "#0c5597", fontWeight: 800 }, greenText: { color: "#166534" }, redText: { color: "#b42318" },
