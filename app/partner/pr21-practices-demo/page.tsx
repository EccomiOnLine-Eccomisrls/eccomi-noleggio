...
export default function Pr21PracticesDemo() {
  ...
  const requestDeactivateCollaborator = () => {
    ...
    <>
      ...
      <section style={s.twoCol}>
        <article style={s.panel}>
          <span style={s.kicker}>OPERATIVITÀ</span>
          <h2 style={s.h2}>Cosa richiede attenzione</h2>
          <div style={practice.status === "SENT_TO_PARTNER" ? s.attention : s.doneSoft}>
            {practice.status === "SENT_TO_PARTNER" ? "1 pratica da prendere in carico." : practice.status === "NEEDS_INFO" ? "1 pratica in attesa di integrazione cliente." : practice.status === "DELIVERED" ? "Nessuna pratica aperta." : "1 pratica in lavorazione."}
          </div>
        </article>
        <article style={s.panel}>
          <span style={s.kicker}>SICUREZZA</span>
          <h2 style={s.h2}>Perimetro della società</h2>
          <p style={s.muted}>Offerte, clienti, documenti, commissioni e collaboratori restano confinati alla tua organizzazione.</p>
        </article>
      </section>
    </>
  );
    ...
    <div style={s.stack}>
      <section style={s.panel}>
        <div style={s.sectionHead}>
          <div><span style={s.kicker}>NUOVA QUOTAZIONE</span><h2 style={s.h2}>Carica il PDF del noleggiatore</h2><p style={s.muted}>La quotazione viene inviata a ECCOMI per verifica. Il Partner non può approvarla o pubblicarla direttamente.</p></div>
          {quoteSubmitted ? <span style={s.status}>IN VERIFICA ECCOMI</span> : null}
        </div>
        <div style={s.quoteComposer}>
          <label style={s.uploadBox}>
            <input type="file" accept="application/pdf,.pdf" style={s.hiddenInput} onChange={(event) => setQuoteFileName(event.target.files?.[0]?.name ?? "")} />
            <span style={s.uploadIcon}>⬆️</span>
            <span><strong>{quoteFileName || "Seleziona quotazione PDF"}</strong><small style={s.block}>PDF · massimo 15 MB nella versione reale</small></span>
          </label>
      ...
      <section style={s.panel}>
        ...
        <article style={s.offerCard}>
          <div style={s.cardTop}><div><span style={s.kicker}>OFFERTA {practice.offerNumber}</span><h3 style={s.h3}>{practice.vehicle}</h3><p style={s.muted}>{practice.version}</p></div><span style={s.status}>{offerStatus}</span></div>
          <div style={s.offerMeta}><div><span style={s.summaryLabel}>Scadenza</span><strong>03/09/2026</strong></div><div><span style={s.summaryLabel}>Gestione</span><strong>Partner</strong></div><div><span style={s.summaryLabel}>Stato</span><strong>{offerStatus === "PUBBLICATA" ? "Visibile" : "Non visibile"}</strong></div></div>
          <div style={s.actions}><button type="button" style={s.secondary} onClick={toggleOffer}>{offerStatus === "PUBBLICATA" ? "Sospendi" : "Riattiva"}</button></div>
        </article>
      </section>
    </div>
  );
  ...
  const renderPractices = () => (
    <section style={s.grid}>
      ...
      <article style={s.detailPanel}>
        ...
        <div style={s.summary}>
          <div style={s.summaryItem}><span style={s.summaryLabel}>Cliente</span><strong style={s.summaryValue}>{practice.customer}</strong></div>
          <div style={s.summaryItem}><span style={s.summaryLabel}>Provincia</span><strong style={s.summaryValue}>{practice.province}</strong></div>
          <div style={s.summaryItem}><span style={s.summaryLabel}>Documenti</span><strong style={s.summaryValue}>{practice.documents}</strong></div>
        </div>
  ...
  const renderCommissions = () => (
    <section style={s.panel}>
      <div style={s.sectionHead}><div><span style={s.kicker}>COMMISSIONI</span><h2 style={s.h2}>Il tuo maturato</h2></div><span style={s.count}>{practice.status === "DELIVERED" ? 1 : 0}</span></div>
      {practice.status === "DELIVERED" ? <article style={s.commissionCard}><div><strong>{practice.vehicle}</strong><p style={s.muted}>Pratica {practice.id}</p></div><div><span style={s.summaryLabel}>Stato</span><strong style={s.greenText}>MATURATA ALLA CONSEGNA</strong></div><div><span style={s.summaryLabel}>Importo</span><strong>Secondo accordi Partner</strong></div></article> : <div style={s.infoBox}>Nessuna commissione maturata. La commissione matura quando la pratica arriva a “Veicolo consegnato”.</div>}
    </section>
  );
  ...
  const renderCollaborators = () => (
    <div style={s.stack}>
      <section style={s.panel}>
        <div style={s.sectionHead}>
          <div><span style={s.kicker}>COLLABORATORI</span><h2 style={s.h2}>Persone della tua società</h2><p style={s.muted}>Il Partner Admin gestisce la propria sottorete senza uscire dal perimetro della società.</p></div>
          <button type="button" style={s.primary} onClick={() => setShowInvite((current) => !current)}>{showInvite ? "Chiudi" : "+ Invita collaboratore"}</button>
        </div>

        {showInvite ? <div style={s.inviteCard}>
          <div style={s.formGrid}>
            <label style={s.fieldLabel}>Nome e cognome<input style={s.input} value={inviteName} onChange={(event) => setInviteName(event.target.value)} placeholder="Es. Mario Rossi" /></label>
            <label style={s.fieldLabel}>Email<input style={s.input} type="email" value={inviteEmail} onChange={(event) => setInviteEmail(event.target.value)} placeholder="nome@azienda.it" /></label>
            <label style={s.fieldLabel}>Ruolo<select style={s.input} value={inviteRole} onChange={(event) => setInviteRole(event.target.value)}><option>Collaboratore</option><option>Operatore Partner</option></select></label>
          </div>
          <div style={s.actions}><button type="button" style={s.primary} onClick={inviteCollaborator}>Prepara invito</button><button type="button" style={s.reset} onClick={() => setShowInvite(false)}>Annulla</button></div>
        </div> : null}

        <div style={s.collaboratorList}>
          <article style={s.collaboratorCard}>
            <div><strong>Partner Admin Demo</strong><p style={s.muted}>partner.admin@example.it</p></div>
            <div><span style={s.summaryLabel}>Ruolo</span><strong>Partner Admin</strong></div>
            <div><span style={s.summaryLabel}>Stato</span><strong style={collaboratorActive ? s.greenText : s.redText}>{collaboratorActive ? "ATTIVO" : "DISATTIVATO"}</strong></div>
            <button type="button" style={collaboratorActive ? s.dangerOutline : s.secondary} onClick={collaboratorActive ? requestDeactivateCollaborator : reactivateCollaborator}>{collaboratorActive ? "Disattiva" : "Riattiva"}</button>
          </article>

          {invitedCollaborators.map((collaborator, index) => <article key={`${collaborator.email}-${index}`} style={s.collaboratorCard}>
            <div><strong>{collaborator.name}</strong><p style={s.muted}>{collaborator.email}</p></div>
            <div><span style={s.summaryLabel}>Ruolo</span><strong>{collaborator.role}</strong></div>
            <div><span style={s.summaryLabel}>Stato</span><strong style={s.greenText}>INVITO PRONTO</strong></div>
            <span style={s.miniBadge}>DEMO</span>
          </article>)}
        </div>
      </section>
    </div>
    ...
    <main style={s.page}>
      ...
      <div style={s.content}>
        <section style={s.hero}>
          <div><span style={s.kicker}>PARTNER ADMIN</span><h1 style={s.title}>{tabMeta[activeTab].title}</h1><p style={s.muted}>{tabMeta[activeTab].description}</p></div>
          <div style={s.scope}><strong>Area protetta</strong><small>Accesso riservato alla tua organizzazione</small></div>
        </section>

        <nav style={s.nav} aria-label="Area Partner">
          {tabs.map((tab) => <button key={tab} type="button" style={activeTab === tab ? s.navActive : s.navGhost} onClick={() => selectTab(tab)}>{tabMeta[tab].label}</button>)}
        </nav>
      ...
      {showDeactivateConfirm ? <div style={s.modalOverlay} role="dialog" aria-modal="true" aria-label="Conferma disattivazione collaboratore">
        <div style={s.modalCard}>
          <span style={s.kicker}>CONFERMA RICHIESTA</span>
          <h2 style={s.modalTitle}>Disattivare questo collaboratore?</h2>
          <p style={s.muted}>Il collaboratore perderà l’accesso all’Area Partner della tua organizzazione. Potrai riattivarlo successivamente.</p>
          <div style={s.warningBox}><strong>Partner Admin Demo</strong><span>partner.admin@example.it</span></div>
          <div style={s.modalActions}><button type="button" style={s.secondary} onClick={() => setShowDeactivateConfirm(false)}>Annulla</button><button type="button" style={s.danger} onClick={confirmDeactivateCollaborator}>Conferma disattivazione</button></div>
        </div>
      </div> : null}
    </main>
  );
...
const s: Record<string, React.CSSProperties> = {
  ...
  stack: { display: "grid", gap: 14 }, kpiGrid: { display: "grid", gridTemplateColumns: "repeat(4,minmax(0,1fr))", gap: 14 }, kpiCard: { display: "grid", gap: 8, padding: 20, borderRadius: 18, background: "#fff", border: "1px solid #dce6f1" }, kpiValue: { fontSize: 26 },
  ...
  sectionHead: { display: "flex", justifyContent: "space-between", gap: 14, alignItems: "center", marginBottom: 14, flexWrap: "wrap" }, count: { minWidth: 32, height: 32, display: "grid", placeItems: "center", borderRadius: 999, background: "#eef6ff", color: "#0c5597", fontWeight: 900 }, h2: { margin: "7px 0 0", fontSize: 24 }, h3: { margin: "8px 0", fontSize: 22 },
