...
import { Building2, CarFront, CheckCircle2, FileText, Gauge, Loader2, LockKeyhole, LogOut, ShieldCheck, UsersRound, WalletCards } from "lucide-react";
...
type Promotion = { id: string; offerNumber: string; brand: string; model: string; version: string; price: string; status: string; statusLabel: string; validUntil: string; days: string };
...
export default function PartnerPage() {
  ...
  useEffect(() => { void loadPortal().catch((loadError) => { setError(loadError instanceof Error ? loadError.message : "Accesso non disponibile."); setChecking(false); }); }, []);
    ...
    return (
      <main style={styles.loginPage}>
        <section style={styles.loginCard}>
          ...
          <div><span style={styles.kicker}><ShieldCheck size={16} /> ACCESSO PARTNER</span><h1 style={styles.loginTitle}>Entra nel tuo spazio operativo</h1><p style={styles.muted}>L’Area Partner è separata dalla Dashboard CEO. Ogni account vede esclusivamente la propria società.</p></div>
          ...
          <div style={styles.securityNote}><ShieldCheck size={18} /><span>Il login CEO non abilita questa area. Serve un account Partner o Partner Admin.</span></div>
        </section>
      </main>
    );
  }
  ...
  const nav: Array<{ id: Section; label: string }> = [
    { id: "overview", label: "Panoramica" }, { id: "offers", label: "Offerte" }, { id: "practices", label: "Pratiche" }, { id: "commissions", label: "Commissioni" },
    ...(isAdmin ? [{ id: "team" as const, label: "Collaboratori" }] : []),
  ];
    ...
    <main style={styles.page}>
      ...
      <div style={styles.content}>
        <section style={styles.hero}>
          ...
          <div style={styles.scopeBadge}><Building2 size={18} /><span><strong>Solo la tua società</strong><small>{session.partner.id}</small></span></div>
        </section>
        ...
        <section style={styles.kpiGrid}>
          ...
          <button style={styles.kpiCard} onClick={() => setSection("commissions")}><WalletCards size={20} /><span>Commissioni</span><strong>{money.format(dashboard.stats.commissionCents / 100)}</strong><small>maturato registrato</small></button>
        ...
        <nav style={styles.nav}>{nav.map((item) => <button key={item.id} type="button" onClick={() => setSection(item.id)} style={{ ...styles.navButton, ...(section === item.id ? styles.navButtonActive : {}) }}>{item.label}</button>)}</nav>
        {error ? <div style={styles.error}>{error}</div> : null}
        ...
        {section === "overview" ? <section style={styles.twoCol}>
          ...
          <article style={styles.panel}><span style={styles.kicker}>SICUREZZA</span><h2 style={styles.panelTitle}>Perimetro della società</h2><p style={styles.muted}>Offerte, clienti, documenti e commissioni sono filtrati server-side sul Partner <strong>{session.partner.name}</strong>. Un URL di un’altra società viene rifiutato.</p></article>
        </section> : null}

        {section === "offers" ? <section style={styles.panel}><div style={styles.sectionHead}><div><span style={styles.kicker}>LE TUE OFFERTE</span><h2 style={styles.panelTitle}>Quotazioni e pubblicazioni</h2></div><span style={styles.countBadge}>{dashboard.promotions.length}</span></div><div style={styles.tableWrap}><table style={styles.table}><thead><tr><th>Offerta</th><th>Veicolo</th><th>Stato</th><th>Scadenza</th></tr></thead><tbody>{dashboard.promotions.map((item) => <tr key={item.id}><td><strong>{item.offerNumber}</strong></td><td>{item.brand} {item.model}<small style={styles.tableSmall}>{item.version}</small></td><td><span style={styles.statusBadge}>{item.statusLabel}</span></td><td>{date.format(new Date(`${item.validUntil}T12:00:00Z`))}</td></tr>)}</tbody></table></div>{!dashboard.promotions.length ? <p style={styles.muted}>Nessuna offerta associata alla tua società.</p> : null}</section> : null}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  primaryButton: { minHeight: 46, border: 0, borderRadius: 11, padding: "11px 16px", display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 8, background: "#1478bd", color: "#fff", fontWeight: 900, cursor: "pointer" }, error: { padding: 13, borderRadius: 11, background: "#fff1f2", color: "#b42318", fontWeight: 800 }, securityNote: { display: "flex", gap: 9, padding: 13, borderRadius: 12, background: "#eff6ff", color: "#0c5597", fontSize: 13, lineHeight: 1.45 },
