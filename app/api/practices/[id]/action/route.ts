...
export async function POST(
  ...
  try {
    ...
    if (!nextStatus) {
      if (!note) {
        return Response.json({ error: "Inserisci una nota operativa." }, { status: 422 });
      }
    ...
    if (!possible.includes(nextStatus) && !internalDirectQuote) {
      return Response.json(
        {
          error: `Passaggio da ${practice.status} a ${nextStatus || "stato non valido"} non consentito.`,
        },
        { status: 422 },
      );
    }

    if (actor.role === "PARTNER" && !partnerAllowedStatuses.has(nextStatus)) {
      return Response.json({ error: "Azione non consentita al partner." }, { status: 403 });
    }

    if (nextStatus === "NEEDS_INFO" && note.length < 5) {
      return Response.json(
        { error: "Descrivi nella nota quali informazioni o documenti mancano." },
        { status: 422 },
      );
    }
    ...
    await db.insert(hubEvents).values({
      ...
      entityType: "lead",
      entityId: id,
      title: `${id} · ${statusLabels[nextStatus] || nextStatus}`,
      payloadJson: JSON.stringify({
        from: practice.status,
        to: nextStatus,
        note: note || null,
