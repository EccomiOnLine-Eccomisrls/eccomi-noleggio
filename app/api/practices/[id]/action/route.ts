...
export async function POST(
  ...
  try {
    ...
    const note = typeof body.note === "string"
      ...
      : "";
    const requestedPriority = typeof body.priority === "string"
      ? body.priority.trim().toUpperCase()
      : "";
    const hasAssignmentRequest = Object.prototype.hasOwnProperty.call(body, "assignedTo");
    const requestedAssignedTo = typeof body.assignedTo === "string"
      ? body.assignedTo.trim().slice(0, 160)
      : "";
    const requestedTrashAction = typeof body.trashAction === "string"
    ...
    const [practicePartner] = await db
      .select()
      .from(partners)
      .where(eq(partners.id, practice.partnerId))
      .limit(1);
    const internalEccomi = isInternalEccomiPartner(practicePartner);

    if (requestedTrashAction) {
      if (actor.role !== "CEO") {
        return Response.json({ error: "Solo il CEO può eliminare una pratica." }, { status: 403 });
      }
      if (requestedTrashAction !== "TRASH") {
        return Response.json({ error: "Azione cestino non valida." }, { status: 422 });
      }
      if (practice.deletedAt) {
        return Response.json({ error: "La pratica è già presente nel cestino." }, { status: 409 });
      }
      if (deleteReason.length < 5) {
        return Response.json({ error: "Indica il motivo dell'eliminazione." }, { status: 422 });
      }
    ...
    if (hasAssignmentRequest) {
      if (actor.role !== "CEO") {
        return Response.json({ error: "Solo il CEO può assegnare una pratica." }, { status: 403 });
      }
      ...
      await db.insert(auditLogs).values({
        id: crypto.randomUUID(),
        actorEmail: actor.email,
        action: nextAssignedTo ? "PRACTICE_ASSIGNED" : "PRACTICE_UNASSIGNED",
        entityType: "lead",
        entityId: id,
      ...
      await db.insert(hubEvents).values({
        id: crypto.randomUUID(),
        eventType: nextAssignedTo ? "NOLEGGIO_PRACTICE_ASSIGNED" : "NOLEGGIO_PRACTICE_UNASSIGNED",
        ecosystem: "ECCOMI_NOLEGGIO",
        entityType: "lead",
        entityId: id,
        title: nextAssignedTo ? `${id} · assegnata a ${nextAssignedTo}` : `${id} · assegnazione rimossa`,
        payloadJson: JSON.stringify({ from: previousAssignedTo, to: nextAssignedTo, actorRole: actor.role }),
        actorEmail: actor.email,
      });
    ...
    if (requestedPriority) {
      if (actor.role !== "CEO") {
        return Response.json({ error: "Solo il CEO può modificare la priorità." }, { status: 403 });
      }
      const allowedPriorities = new Set(["LOW", "NORMAL", "HIGH"]);
      if (!allowedPriorities.has(requestedPriority)) {
        return Response.json({ error: "Priorità non valida." }, { status: 422 });
      }
      const now = new Date().toISOString();
      await db.update(leads).set({ priority: requestedPriority, updatedAt: now }).where(eq(leads.id, id));
      await db.insert(auditLogs).values({
        id: crypto.randomUUID(), actorEmail: actor.email, action: "PRACTICE_PRIORITY_CHANGED",
      ...
      await db.insert(hubEvents).values({
        id: crypto.randomUUID(), eventType: "NOLEGGIO_PRACTICE_PRIORITY_CHANGED", ecosystem: "ECCOMI_NOLEGGIO",
        entityType: "lead", entityId: id, title: `Priorità aggiornata · ${id}`,
        payloadJson: JSON.stringify({ from: practice.priority || "NORMAL", to: requestedPriority, actorRole: actor.role }), actorEmail: actor.email,
      });
    ...
    if (!nextStatus) {
      ...
      await db.insert(hubEvents).values({
        id: crypto.randomUUID(), eventType: "NOLEGGIO_PRACTICE_NOTE", ecosystem: "ECCOMI_NOLEGGIO",
        entityType: "lead", entityId: id, title: `Nota aggiunta alla pratica ${id}`,
        payloadJson: JSON.stringify({ status: practice.status, actorRole: actor.role }), actorEmail: actor.email,
      });
      return Response.json({ ok: true, status: practice.status, noteAdded: true });
    }

    const possible = allowedTransitions[practice.status] || [];
    const internalDirectQuote = actor.role === "CEO"
      && internalEccomi
      && practice.status === "ECCOMI_REVIEW"
      && nextStatus === "QUOTE";
