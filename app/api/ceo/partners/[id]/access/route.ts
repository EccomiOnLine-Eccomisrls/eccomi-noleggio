...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!user || !["PARTNER", "PARTNER_ADMIN"].includes(user.role)) {
    return redirectBack(request, id, { accessError: "Account Partner non trovato." });
  }
  ...
  await db.insert(auditLogs).values({
    ...
    action: "PARTNER_ACCESS_DISABLED",
    entityType: "partner_user",
    entityId: email,
    payloadJson: JSON.stringify({ partnerId: id, role: user.role }),
    createdAt: now,
  });
