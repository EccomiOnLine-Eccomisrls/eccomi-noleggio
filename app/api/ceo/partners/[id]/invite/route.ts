...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (isRenderPullRequestPreview(request)) return redirectBack(request, id, { invitePreview: "1", inviteEmail: email });
    ...
    if (existing?.partnerId && existing.partnerId !== id) return redirectBack(request, id, { accessError: "Questa email è già collegata a un'altra società Partner." });
    if (existing && !["PARTNER", "PARTNER_ADMIN"].includes(existing.role)) return redirectBack(request, id, { accessError: "Questa email appartiene a un account ECCOMI interno e non può essere usata come Partner." });
    if (existing?.active) return redirectBack(request, id, { accessError: "L'account è già attivo. Disattivalo prima di inviare un nuovo link di attivazione." });

    const generated = await generatePartnerActivationToken({ email, displayName, partnerId: id, existingAccount: Boolean(existing) });
    const now = new Date().toISOString();
    await db.insert(users).values({ email, displayName, role: "PARTNER_ADMIN", partnerId: id, active: false, createdAt: now, updatedAt: now }).onConflictDoUpdate({
      target: users.email,
    ...
    const emailResult = await sendPartnerActivationEmail({ email, displayName, partnerName: partner.name, activationUrl: activationUrl.toString(), resend: Boolean(existing) });
    ...
    await db.insert(auditLogs).values({
      ...
      payloadJson: JSON.stringify({ partnerId: id, role: "PARTNER_ADMIN", resendId: emailResult.id, linkType: generated.type, active: false }), createdAt: now,
    });
