...
function enabled(value: string | undefined) {
  return ["true", "1", "yes", "si", "sì", "on"].includes(
...
export function normalizeShopDomain(value: string) {
  ...
  if (!/^[a-z0-9][a-z0-9-]*\.myshopify\.com$/.test(candidate)) {
    throw new Error(
      "Inserisci il dominio Shopify nel formato nome-negozio.myshopify.com.",
    );
  }
...
async function acquireClientCredentialsToken(
  ...
  if (!response.ok || !payload.access_token) {
    if (response.status === 401 || response.status === 403) {
      throw new Error(
        "Shopify ha rifiutato Client ID o Client secret. Controlla i due valori e riprova.",
      );
    }

    if (payload.error === "shop_not_permitted") {
      throw new Error(
        "L’app e il negozio Shopify non risultano nella stessa organizzazione.",
      );
    }

    throw new Error(
      payload.error_description ||
      "Shopify non ha autorizzato il collegamento. Riprova tra poco.",
    );
  }

  const expiresIn = Math.max(
    300,
    Number(payload.expires_in || 86_399),
  );
...
async function graphqlWithConfiguration<T>(
  ...
  if (!response.ok) {
    throw new Error(
      `Shopify ha risposto con errore ${response.status}.`,
    );
  }
...
async function configuration(): Promise<ShopifyConfiguration> {
  ...
  const staticPublication =
    runtime.SHOPIFY_ONLINE_STORE_PUBLICATION_ID?.trim();
  ...
  if (staticShop && staticToken && staticPublication) {
    ...
    return {
      ...
      apiVersion:
        runtime.SHOPIFY_API_VERSION?.trim() || "2026-07",
      templateSuffix:
        runtime.SHOPIFY_NOLEGGIO_TEMPLATE_SUFFIX?.trim() ||
        "eccomi-noleggio",
      collectionHandle:
        runtime.SHOPIFY_NOLEGGIO_COLLECTION_HANDLE?.trim() ||
        "eccomi-noleggio",
  ...
  if (!record || record.status !== "CONNECTED") {
    throw new Error(
      "Collegamento Shopify non ancora configurato.",
    );
  }

  const clientSecret = await decryptCredential(
  ...
  return {
    ...
    collectionHandle:
      ...
      "eccomi-noleggio",
    storefrontBaseUrl:
      record.storefrontUrl ||
      `https://${record.shopDomain}`,
    requestBaseUrl:
      runtime.PUBLIC_REQUEST_BASE_URL?.trim() || "",
  };
}
...
export async function getShopifyConnectionStatus(): Promise<ShopifyConnectionStatus> {
  const runtime = getRuntimeEnv();
  const publishingReady = enabled(
    runtime.SHOPIFY_PUBLISHING_ENABLED,
  );

  if (
    runtime.SHOPIFY_SHOP_DOMAIN &&
    runtime.SHOPIFY_ADMIN_ACCESS_TOKEN &&
    runtime.SHOPIFY_ONLINE_STORE_PUBLICATION_ID
  ) {
    return {
      connected: true,
      publishingReady,
  ...
  return {
    ...
    publicationLabel:
      record?.publicationLabel || null,
    clientIdHint: record
      ? clientIdHint(record.clientId)
      : null,
...
export async function isShopifyConfigured() {
  return (
    await getShopifyConnectionStatus()
  ).connected;
}
...
export async function connectShopifyIntegration(input: {
  ...
  if (clientId.length < 12 || clientId.length > 240) {
    throw new Error(
      "Il Client ID Shopify non sembra completo.",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Il Client secret Shopify non sembra completo.",
    );
  }
  ...
  const verification =
    await graphqlWithConfiguration<{
      shop: {
        name: string;
      ...
      verificationConfig,
      `query VerifyEccomiNoleggioConnection {
        shop {
          name
          myshopifyDomain
          primaryDomain {
  ...
  const verifiedDomain = normalizeShopDomain(
    verification.shop.myshopifyDomain,
  );

  if (verifiedDomain !== shopDomain) {
    throw new Error(
      "Il Client ID appartiene a un negozio Shopify diverso.",
    );
  }

  const onlineStore =
    verification.publications.nodes.find(
      (publication) =>
        publication.supportsFuturePublishing,
    ) ||
    verification.publications.nodes.find(
      (publication) =>
        /online store|negozio online/i.test(
  ...
  if (!onlineStore) {
    throw new Error(
      "Il canale Negozio online non è stato trovato tra le pubblicazioni Shopify.",
    );
  }
  ...
  const encryptedClientSecret =
    await encryptCredential(clientSecret);
  ...
  await getDb()
    ...
    .values({
      ...
      storefrontUrl:
        verification.shop.primaryDomain?.url ||
        `https://${shopDomain}`,
      clientId,
      encryptedClientSecret,
      publicationId: onlineStore.id,
      publicationLabel:
        onlineStore.catalog?.title ||
        "Negozio online",
      publicationAutoPublish:
        onlineStore.autoPublish,
      status: "CONNECTED",
...
function titleFor(promotion: ShopifyPromotion) {
  return `${promotion.brand} ${promotion.model} a noleggio lungo termine – ${euro(
...
function descriptionFor(
  ...
  const requestUrl = requestUrlFor(
    requestBaseUrl,
    promotion.id,
  );
  ...
  return [
    `<h2>${escapeHtml(
      promotion.brand,
    )} ${escapeHtml(promotion.model)}</h2>`,

    `<p><strong>${euro(
      promotion.monthlyGrossCents,
    )} al mese IVA inclusa</strong></p>`,

    `<p><strong>Anticipo:</strong> ${euro(
      promotion.depositGrossCents,
    )} · <strong>Durata:</strong> ${
      promotion.durationMonths
    } mesi · <strong>Chilometri:</strong> ${promotion.totalKm.toLocaleString(
      "it-IT",
    )} km</p>`,

    promotion.delivery
      ? `<p><strong>Consegna prevista:</strong> ${escapeHtml(
          promotion.delivery,
        )}</p>`
      : "",
    ...
    requestUrl
      ? `<p><a href="${escapeHtml(
          requestUrl,
        )}" style="display:block;padding:16px 20px;border-radius:12px;background:#075392;color:#ffffff;font-weight:800;text-align:center;text-decoration:none;">AVVIA LA RICHIESTA DI NOLEGGIO →</a></p>`
      : "",
    ...
    `<p>Noleggiatore: ${escapeHtml(
      promotion.provider,
    )} · Offerta ${escapeHtml(
      promotion.offerNumber,
    )} · Valida fino al ${escapeHtml(
...
async function stageProductImage(
  ...
  if (!target) {
    throw new Error(
      "Shopify non ha preparato il caricamento dell'immagine.",
    );
  }
  ...
  if (!upload.ok) {
    throw new Error(
      `Caricamento immagine Shopify non riuscito (${upload.status}).`,
    );
  }
  ...
  return {
    ...
    seo: {
      ...
      description: `${euro(
        ...
        promotion.durationMonths
      } mesi, ${promotion.totalKm.toLocaleString(
        "it-IT",
      )} km, anticipo ${euro(
        promotion.depositGrossCents,
      )}.`,
...
async function collectionState(
  ...
  return graphqlWithConfiguration<{
    ...
    config,
    `query EccomiNoleggioCollection(
      $handle: String!
      $productId: ID!
      $publicationId: ID!
    ) {
...
async function ensureNoleggioCollection(
  config: ShopifyConfiguration,
  productId: string,
) {
  let state = await collectionState(
    config,
    productId,
  );

  let collection =
    state.collectionByHandle;
  ...
  if (!collection) {
    const created =
      await graphqlWithConfiguration<{
        ...
        config,
        `mutation CreateEccomiNoleggioCollection(
        ...
        }`,
        {
          input: {
            ...
            descriptionHtml:
              "<p>Offerte di noleggio a lungo termine verificate da ECCOMI.</p>",
          },
        },
      );
    ...
    if (
      !created.collectionCreate.collection
    ) {
      throw new Error(
        "Shopify non ha creato la collezione Eccomi Noleggio.",
      );
    }

    state = await collectionState(
      config,
      productId,
    );
  ...
  if (!collection) {
    throw new Error(
      "Collezione Eccomi Noleggio non disponibile.",
    );
  }
  ...
  if (
    ...
    const added =
      await graphqlWithConfiguration<{
        ...
        config,
        `mutation AddEccomiNoleggioProduct(
          $id: ID!
          $productIds: [ID!]!
        ) {
          collectionAddProducts(
    ...
    if (
      added.collectionAddProducts.userErrors
        .length
    ) {
      throw new Error(
        added.collectionAddProducts.userErrors
  ...
  if (
    ...
    const published =
      await graphqlWithConfiguration<{
        ...
        config,
        `mutation PublishEccomiNoleggioCollection(
          $id: ID!
          $publicationId: ID!
        ) {
          publishablePublish(
  ...
  if (!verified) {
    throw new Error(
      "Shopify non ha confermato la collezione Eccomi Noleggio.",
    );
  }

  if (!verified.hasProduct) {
    throw new Error(
      "Il prodotto non rispetta le regole della collezione automatica Eccomi Noleggio.",
    );
  }
...
async function configureRentalProductVariants(
  ...
  let variantIds =
    knownVariantIds.filter(Boolean);
  ...
  if (!variantIds.length) {
    ...
    variantIds =
      productData.product?.variants.nodes.map(
        (variant) => variant.id,
      ) || [];
  }
  ...
  if (!variantIds.length) {
    throw new Error(
      "Shopify non ha restituito la variante del prodotto.",
    );
  }
  ...
  const updated =
    await graphqlWithConfiguration<{
      ...
      config,
      `mutation ConfigureRentalProductVariants(
        $productId: ID!
  ...
  if (
    updated.productVariantsBulkUpdate
      .userErrors.length
  ) {
    throw new Error(
      updated.productVariantsBulkUpdate
  ...
  const incomplete =
    updated.productVariantsBulkUpdate.productVariants.find(
      (variant) =>
        variant.price !== "0.00" ||
  ...
  if (incomplete) {
    throw new Error(
      "Shopify non ha applicato tutte le regole ECCOMI NOLEGGIO al prodotto.",
    );
  }
}

export async function createPromotionDraftOnShopify(
  promotion: ShopifyPromotion,
  cover: ProductCover,
) {
  ...
  const created =
    await graphqlWithConfiguration<{
      ...
      config,
      `mutation CreateNoleggioProduct(
        ...
        media: [
          {
            ...
            alt: `${promotion.brand} ${promotion.model} – immagine illustrativa`,
          },
        ],
      },
    );
  ...
  if (
    created.productCreate.userErrors.length
  ) {
    throw new Error(
...
export async function publishPreparedPromotionToShopify(
  productId: string,
) {
  const config = await configuration();
  ...
  const collection =
    await ensureNoleggioCollection(
      config,
      productId,
    );
  ...
  const updated =
    await graphqlWithConfiguration<{
      ...
      config,
      `mutation ActivateNoleggioProduct(
  ...
  if (!product) {
    throw new Error(
      "Shopify non ha confermato l’attivazione del prodotto.",
    );
  }
  ...
  if (!config.publicationAutoPublish) {
    const published =
      await graphqlWithConfiguration<{
        ...
        config,
        `mutation PublishNoleggioProduct(
          $id: ID!
          $publicationId: ID!
        ) {
          publishablePublish(
  ...
  return {
    productId: product.id,
    handle: product.handle,
    url: `${config.storefrontBaseUrl.replace(
      /\/$/,
      "",
...
export async function updateExistingPromotionOnShopify(
  productId: string,
  promotion: ShopifyPromotion,
  cover: ProductCover,
) {
  ...
  const updated =
    await graphqlWithConfiguration<{
      ...
      config,
      `mutation UpdateExistingNoleggioProduct(
        $product: ProductUpdateInput!
      ) {
        productUpdate(product: $product) {
          product {
  ...
  if (!config.publicationAutoPublish) {
    const published =
      await graphqlWithConfiguration<{
        ...
        config,
        `mutation RepublishUpdatedNoleggioProduct(
          $id: ID!
          $publicationId: ID!
        ) {
          publishablePublish(
    ...
    const publicationErrors =
      published.publishablePublish.userErrors.filter(
        (error) =>
          !/already published/i.test(
            error.message,
          ),
      );
...
export async function unpublishPromotionFromShopify(
  productId: string,
) {
  const config = await configuration();
  ...
  const data =
    await graphqlWithConfiguration<{
      ...
      config,
      `mutation UnpublishNoleggioProduct(
        $id: ID!
        $publicationId: ID!
      ) {
        publishableUnpublish(
