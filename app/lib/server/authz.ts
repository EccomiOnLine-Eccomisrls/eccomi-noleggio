...
function headerName(request: Request) {
...
export async function getActor(request: Request): Promise<Actor | null> {
  ...
  const email = headerEmail || (isLocalRequest(request) ? (runtime.CEO_EMAIL || "ceo@eccomi.local") : "");
  if (!email) return null;

  if (!headerEmail && isLocalRequest(request) && !runtime.CEO_EMAIL) {
    return {
      email,
      displayName: headerName(request) || "Salvatore Del Libano",
      role: "CEO",
      partnerId: null,
    };
  }
  return getActorForIdentity(email, headerName(request) || "Utente ECCOMI");
}

export async function requireActor(request: Request): Promise<Actor> {
  const actor = await getActor(request);
  if (!actor) throw new Response(JSON.stringify({ error: "Accesso non autorizzato." }), {
    status: 401,
...
export async function requireCeo(request: Request): Promise<Actor> {
  ...
  if (actor.role !== "CEO") throw new Response(JSON.stringify({ error: "Azione riservata al CEO." }), {
    status: 403,
    headers: { "content-type": "application/json; charset=utf-8" },
  });
  return actor;
